-- Compact, idempotent InstantMeeting lead-form + paid-signup Meta CAPI schema.
-- Safe to run once on an existing InstantMeeting Supabase project if these pieces are missing.
...
ALTER TABLE waiting_guests ADD COLUMN IF NOT EXISTS qualification_verdict TEXT CHECK (qualification_verdict IN ('qualified','unqualified','review'));
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_waiting_guests_lead_session ON waiting_guests(lead_session_token) WHERE lead_session_token IS NOT NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_pending_signups_email_pending ON pending_signups(email) WHERE status = 'pending';
...
CREATE POLICY "Hosts manage their lead forms" ON lead_forms FOR ALL USING (user_id::text = auth.uid()::text);
DROP POLICY IF EXISTS "Anyone can view active lead forms" ON lead_forms;
CREATE POLICY "Anyone can view active lead forms" ON lead_forms FOR SELECT USING (is_active = true);

DROP POLICY IF EXISTS "Hosts manage their form questions" ON lead_form_questions;
CREATE POLICY "Hosts manage their form questions" ON lead_form_questions FOR ALL USING (
    EXISTS (SELECT 1 FROM lead_forms WHERE lead_forms.id = lead_form_questions.form_id AND lead_forms.user_id::text = auth.uid()::text)
);
DROP POLICY IF EXISTS "Anyone can view questions for active forms" ON lead_form_questions;
CREATE POLICY "Anyone can view questions for active forms" ON lead_form_questions FOR SELECT USING (
    EXISTS (SELECT 1 FROM lead_forms WHERE lead_forms.id = lead_form_questions.form_id AND lead_forms.is_active = true)
);
