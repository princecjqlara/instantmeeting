...
test('iPhone Facebook in-app browser prefers external open but keeps current-browser fallback', () => {
    const strategy = getInAppBrowserOpenStrategy(
        'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 [FBAN/FBIOS;FBAV/460.0.0.42.109;FBBV/123456]'
    )
...
test('Android in-app browser auto-attempts external open first', () => {
    const strategy = getInAppBrowserOpenStrategy(
        'Mozilla/5.0 (Linux; Android 14; Pixel 8) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/120.0.0.0 Mobile Safari/537.36 [FBAN/FB4A;FBAV/444.0.0.0.0;]'
    )

    assert.deepEqual(strategy, {
        autoAttemptExternal: true,
        showExternalOption: true,
        showCurrentBrowserOption: true,
    })
})
